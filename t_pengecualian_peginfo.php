...
class ct_pengecualian_peg extends cTable {
	...
	function __construct() {
		...
		// tipe
		$this->tipe = new cField('t_pengecualian_peg', 't_pengecualian_peg', 'x_tipe', 'tipe', '`tipe`', '`tipe`', 16, -1, FALSE, '`tipe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `pegawai_nama` FROM `pegawai` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`pegawai_id` = `t_pengecualian_peg`.`pegawai_id` LIMIT 1) AS `EV__pegawai_id` FROM `t_pengecualian_peg`" .
			") `EW_TMP_TABLE`";
